...
/// Minimal valid single-file torrent; first pieces-hash byte = `tag` so
/// distinct tags yield distinct info-hashes.
...
async fn shared_listener_routes_inbound_by_info_hash() {
    ...
    let mgr = SessionManager::with_shared(registry, None, port, acceptor_task);
