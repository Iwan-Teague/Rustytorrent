...
| Per-peer task | [`peer/connection.rs`](../src/peer/connection.rs) | `tokio::select!` between socket reads, command channel, and keep-alive timer; B3 per-peer inbound-Request token bucket; writes bounded by WRITE_STALL_TIMEOUT. No library wraps this. |
| Solicitation + reject validation | engine.rs Block/RejectRequest arms | Only blocks actually requested from the sending peer are accepted (unsolicited drops silently); BEP 6 REJECT_REQUEST is range- AND ownership-checked, and a SHA1 failure bans the sender's IP and tears down all of its engine-side state. |
...
| Shared acceptor (daemon) | [`acceptor.rs`](../src/acceptor.rs) | One inbound listener for every hosted torrent: drives each handshake far enough to learn the info-hash, routes to the owning session's channel via a shared registry. |
| Interface bind (kill switch) | [`netbind.rs`](../src/netbind.rs) | `SO_BINDTODEVICE` (Linux) / `IP_BOUND_IF` (macOS, BSD) dial + UDP-bind helpers so `--bind-iface` traffic fails closed if the interface vanishes; also resolves an interface's local IP for reqwest. |
| Per-peer rate limiters | [`ratelimit.rs`](../src/ratelimit.rs) | Token buckets used for the B3 inbound-Request cap, the B4 per-IP connect-flood gate, and engine-wide `--max-down`/`--max-up` throttles (capacity floored at one block). |
| OS sandbox | [`sandbox.rs`](../src/sandbox.rs) | `--sandbox`: Linux seccomp BPF allow-list installed before the download loop; macOS SBPL deny-default profile; unsupported platforms refuse rather than no-op. |
| µTP packet codec | [`peer/utp/packet.rs`](../src/peer/utp/packet.rs) | BEP 29 header + extension chain (selective ack), wrap-safe seq math, `Payload` slices sharing one allocation per application write. |
| µTP connection state machine | [`peer/utp/connection.rs`](../src/peer/utp/connection.rs) | SYN/STATE/DATA/FIN transitions, cumulative + selective acks, RTO with exponential backoff and fast retransmit, LEDBAT delay-based window, randomized receiver seq as anti-spoof accept token, receive-window enforcement bounding undelivered inbound memory. |
| µTP socket runtime | [`peer/utp/socket.rs`](../src/peer/utp/socket.rs) | Single driver task multiplexing all connections over one UDP socket; per-connection `SendGate` credit ledger bounds the app→driver send path; bounded delivery queue caps driver→stream memory; forged-SYN connection cap. |
